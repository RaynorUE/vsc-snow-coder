/** 
 * Use the action API to configure UI actions with which users can interact. Use these scripts in the UI Action [sys_ui_action] table. For information, see [UI actions](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/list-administration/task/t_EditingAUIAction.html).
 * 
...
declare class ActionAPIBoth {
	 ...
	 * 
	 * *   [GlideURI](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideURIScopedAPI "Enables handling the URI parameter in scoped applications.")
	 * *   [GlideURI - Global](https://developer.servicenow.com/go_to_api.do?ID=GlideURIGlobalAPI;v=orlando)
	 */
	 ...
	 * For use cases:
	 * 	 * 
	 * 	 * *   [Create a UI routing action](https://docs.servicenow.com/bundle/orlando-application-development/page/script/useful-scripts/task/t_CreateAUIRoutingAction.html)
	 * 	 * *   [Create knowledge articles from HR cases](https://docs.servicenow.com/bundle/orlando-servicenow-platform/page/product/knowledge-management/task/ui-action-hrcase-article.html)
	 * 	 * *   [Define task relationships with UI actions](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/task-table/reference/r_UIActions.html)
	 *
	 * @param {{[fieldName: string]: string}} URL URL to set as the redirect. You can provide the URL as a string or a [GlideRecord](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI "Scoped GlideRecord is used for database operations."). If you pass the URL as a GlideRecord, this value takes the focus to that record's form.
	 *
	 * @returns {void} Method does not return a value
	 */
	 ...
	 *
	 * @param {{[fieldName: string]: string}} URL URI to set as the return location after a UI action is complete. You can provide the URL as a string or a [GlideRecord](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI "Scoped GlideRecord is used for database operations.").
	 *
	 * @returns {void} Method does not return a value
	 */
...
declare class GlideAggregate {
	 ...
	 *
	 * @param {string} agg Type of the aggregate.
	 * 
	 * Valid values:
	 * 
	 * *   AVG
	 ...
	 *
	 * @returns {string} Value of the aggregate.
	 */
...
declare class GlideCalendarDateTime {
	 ...
	 * 	 * 
	 * 	 * For details on creating a business calendar and setting the time zone value, see [Create a business calendar](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/time/task/create-business-calendar.html).
	 *
	 *
...
declare class GlideElementCurrency2 {
	 ...
	 *
	 * For additional information on FX Currency fields, see [Setting up and operating FX Currency fields](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/currency/concept/fx-currency.html).
	 *
	 *
	 ...
	 * 	 * 
	 * 	 * For additional information on FX Currency and the configuration table, see [Configure FX Currency global settings](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/currency/task/fx-currency-configure.html).
	 *
	 *
	 ...
	 * 	 * 
	 * 	 * For additional information on the values in the FX Currency Configuration [fx_configuration] table, see [Setting up and operating FX Currency fields](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/currency/concept/fx-currency.html).
	 *
	 *
 ...
 * ----------------
 * 
 * GlideFilter is case-sensitive by default. Use the [setCaseSensitive()](dev.do#!/reference/api/orlando/server/ "Enables or disables case-sensitive filter results.") method to enable or disable case sensitivity. [GlideRecord](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI "Scoped GlideRecord is used for database operations.") queries are case-insensitive.
 * 
...
declare class GlideFilter {
	...
	/**
	 *
	 * @param {string} filter Encoded query string in standard Glide Format. Results are case-sensitive, unless disabled using the [setCaseSensitive()](dev.do#!/reference/api/orlando/server/ "Enables or disables case-sensitive filter results.") method.
	 * 
	 ...
	 * @param {string} filter Encoded query string in standard Glide format. See [Encoded query strings](https://docs.servicenow.com/bundle/orlando-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html).
	 * 
	 * To exclude null values from GlideFilter query results, add ISNOTEMPTY to the query condition.
 ...
 *   
 * For an example on using this class, [Example: g_scratchpad](https://developer.servicenow.com/dev.do#!/guides/orlando/now-platform/tpb-guide/client_scripting_technical_best_practices%23%23example-g-scratchpad) on the Developer Site.
 * 
 */
...
declare class GlideQueryCondition {
	 ...
	 *
	 * addOrCondition() works in conjunction with any of the [addQuery()](https://developer.servicenow.com/go_to_api.do?ID=r_ScopedGlideRecordAddQuery_String_String_Object&v=orlando) methods to OR the specified query parameters to the query previously constructed using addQuery().
	 * 	 * 
 ...
 * 
 * For information on a class that performs the same functions as GlideRecord and enforces ACLs, see [Using GlideRecordSecure](https://docs.servicenow.com/bundle/orlando-application-development/page/script/glide-server-apis/topic/p_GlideServerAPIs.html#concepthrq5bcgp).
 * 
 * Always test queries on a sub-production instance prior to deploying them on a production instance. An incorrectly constructed encoded query, such as including an invalid field name, produces an invalid query. When the invalid query is run, the invalid part of the query condition is dropped, and the results are based on the valid part of the query, which may return all records from the table. Using an insert(), update(), deleteRecord(), or deleteMultiple() method on bad query results can result in data loss.
 * 
 * You can set the glide.invalid_query.returns_no_rows system property to true to have queries with invalid encoded queries return no records.
 ...
 * 
 * *   [getValue()](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordGetValue_String "Retrieves the string value of an underlying element in a field.")
 * *   [getDisplayValue()](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordGetDisplayValue "Retrieves the display value for the current record.")
 * 
 ...
 * See also:
 * 
 * *   [GlideAggregate](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideAggregateScopedAPI "GlideAggregate enables you to easily create database aggregation queries.")
 * *   [GlideElement](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideElementScopedAPI "The Scoped GlideElement API provides a number of convenient script methods for dealing with fields and their values. Scoped GlideElement methods are available for the fields of the current GlideRecord.")
 * 
 */
declare class GlideRecord {
	 ...
	 * 	 * You can set the glide.invalid_query.returns_no_rows system property to true to have queries with invalid encoded queries return no records.
	 *
	 * @param {string} query An [encoded query string](https://docs.servicenow.com/bundle/orlando-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html).
	 *
	 * @returns {void} Method does not return a value
	 */
	addEncodedQuery(query: string): void
	 ...
	 * 	 * You can set the glide.invalid_query.returns_no_rows system property to true to have queries with invalid encoded queries return no records.
	 *
	 * @param {string} query An [encoded query string](https://docs.servicenow.com/bundle/orlando-platform-administration/page/use/using-lists/concept/c_EncodedQueryStrings.html) .
	 *
	 ...
	 * 	 * See also:
	 * 	 * 
	 * 	 * *   [GlideElement – toString()](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideElementScopedAPI#r_ScopedGlideElementToString "Converts the value of a GlideRecord field to a string.")
	 * 	 * *   [getElements()](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI#SGR-getElements "Returns an array of GlideElement objects. Each object describes a field in the current GlideRecord.")
	 *
	 ...
	 * 	 * 
	 * 	 * *   [GlideElement – toString()](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideElementScopedAPI#r_ScopedGlideElementToString "Converts the value of a GlideRecord field to a string.")
	 * 	 * *   [getElement()](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordGetElement_String "Retrieves the GlideElement object for the specified field.")
	 *
	 *
	 * @returns {{[fieldName: string]: string}} Array of [GlideElement](dev.do#!/reference/api/orlando/server/c_GlideElementAPI "The GlideElement API provides a number of convenient script methods for dealing with fields and their values. GlideElement methods are available for the fields of the current GlideRecord.") objects. Each object describes a field in the current GlideRecord.
	 */
	 ...
	 *
	 * @returns {boolean} True if the field is defined in the current table.
	 */
	 ...
	 *
	 * Call this method more than once to order by multiple columns. Results are arranged in ascending order. To arrange records in descending order, see [Scoped GlideRecord - orderByDesc(String name)](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordOrderByDesc_String "Specifies a descending orderBy column.").
	 *
	 ...
	 *
	 * Call this method more than once to order by multiple columns. Results are arranged in descending order. To arrange records in ascending order, see [Scoped GlideRecord - orderBy(String name)](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordOrderBy_String "Specifies an orderBy column.").
	 *
	...
	/**
	 *
	 * Runs the query against the table based on the filters specified by various query methods such as addQuery, addEncodedQuery, and so on.
	 *
	 ...
	 *
	 * @param {{[fieldName: string]: string}} field Column name to query on.
	 * @param {{[fieldName: string]: string}} value Value to query for.
	 *
	 * @returns {void} Method does not return a value
	 */
	query(field: {[fieldName: string]: string}, value: {[fieldName: string]: string}): void
	...
	/**
	 *
	 * Runs the query against the table based on the filters specified by the various query methods such as addQuery() and addEncodedQuery(). Use this method on tables where there is a column named query.
	 *
	 ...
	 *
	 * @param {{[fieldName: string]: string}} name Column name on which to query.
	 * @param {{[fieldName: string]: string}} value Value for which to query.
	 *
	 * @returns {void} Method does not return a value
	 */
	_query(name: {[fieldName: string]: string}, value: {[fieldName: string]: string}): void
...
declare class GlideSPScriptable {
	 ...
	 */
	getField(gr: GlideRecord, fieldName: string): {[fieldName: string]: string}
	 ...
	 */
	getFields(gr: GlideRecord, fieldNames: string): {[fieldName: string]: string}
	 ...
	 */
	getFieldsObject(gr: GlideRecord, fieldNames: string): {[fieldName: string]: string}
	 ...
	 *
	 * @param {GlideRecord} gr The record to retrieve Service Catalog variables for. Must be a record with Service Catalog variables defined, such as a requested item [sc_req_item] record or an incident submitted through a record producer.
	 ...
	 */
	getRecordVariablesArray(gr: GlideRecord, includeNilResponses: boolean): {[fieldName: string]: string}
...
declare class GlideSysAttachment {
	 ...
	 *
	 * You can use the [GlideTextReader](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideTextReaderScopedAPI "Provides the ability to read single lines from an input stream. Because an input stream is used, it is not subject to the 5MB attachment size limit.") API to read the content stream.
	 *
...
declare class GlideSystem {
	 ...
	 *
	 * The passed in event schedule information is stored in the Events [sysevent] table. For additional information on events, see [Events](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/platform-events/concept/events.html).
	 *
	 ...
	 * 	 * 
	 * 	 * For additional information about the Message table, see [Message table](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/localization/reference/r_MessageTable.html).
	 * 	 * 
	 ...
	 *
	 * This method has been deprecated. Instead, use the [getTimeZoneName()](https://developer.servicenow.com/go_to_api.do?ID=r_ScopedGlideSessionGetTimeZoneName&v=orlando) method in the GlideSession API.
	 *
	 *
	 ...
	 *
	 * See [GlideUser](dev.do#!/reference/api/orlando/server/c_GlideUserAPI "The GlideUser API provides access to information about the current user and current user roles. Using the GlideUser API avoids the need to use the slower GlideRecord queries to get user information.") for a list of available methods.
	 *
	 *
...
declare class GlideTime {
	 ...
	 *
	 * For information on the time formats that you can pass into this method, see [Global date and time field format](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/time/reference/r_FormatDateAndTimeFields.html). You can pass these values in any order and add separators such as a dash, space, backslash, or colon. In addition, you can also pass the character "a" to return am/pm.
	 *
 ...
 * 
 * *   [Navigate by URL](https://docs.servicenow.com/bundle/orlando-platform-user-interface/page/use/navigation/task/navigate-using-url.html)
 * *   [Navigation stack](https://docs.servicenow.com/bundle/orlando-platform-user-interface/page/administer/navigation-and-ui/concept/c_NavigationStack.html)
 * 
 *   
 *   
 * See also: [GlideURI -Global](https://developer.servicenow.com/go_to_api.do?ID=GlideURIGlobalAPI;v=orlando)
 * 
 */
...
/** 
 * This API requires the Performance Analytics (com.snc.pa.analytics_center) application. For more information, see [Performance Analytics](https://docs.servicenow.com/bundle/orlando-now-intelligence/page/use/performance-analytics/reference/r_PALandingPage.html).
 * 
 */
declare class PAScorecard {
	 ...
	 * 
	 * <indicator sys_id>:<breakdown sys_id>:<element sys_id>:<lvl-2 breakdown sys_id>:<lvl-2 element sys_id>:<aggregate sys_id>:<domain_sys_id>
	 * 
	 ...
	 * 
	 * For information about obtaining the sys_id values of records, see [Unique record identifier (sys_id)](https://docs.servicenow.com/bundle/orlando-platform-administration/page/administer/table-administration/concept/c_UniqueRecordIdentifier.html).
	 * 
	 ...
	 * @param {string} breakdown Enter the sys_id of a breakdown to return chart information organized as defined by the breakdown. For example, enter the sys_id of a priority breakdown to return separate task chart information for each priority value, such as Number of open incidents / Priority / 2 - High.
	 ...
	 * @param {string} display Set to true to return only indicators that are displayed on the Analytics Hub. Set this parameter to all to return all indicators. This parameter is true by default.
	 * @param {string} favorites Set to true to return only indicators that are favorites of the querying user.
	 * @param {string} key Set to true to return results only for key indicators.
	 * @param {string} target Set to true to return results only for indicators that have a target set on the Analytics Hub.
	 * @param {string} contains Enter a comma-separated list of names or descriptions to return results only from indicators with a matching value. Because this list follows AND logic, not OR logic, all entries must match or no results are returned.
	 ...
	 * @param {string} per_page Enter the maximum number of indicators each query can return on a page. By default this value is 10, and the maximum is 100.
	 ...
	 * @param {string} sortby Specify the value to use when sorting results. Valid values for this parameter are value, change, changeperc, gap, gapperc, duedate, name, order, default, group, indicator_group, frequency, target, date, trend, bullet, and direction. By default, queries sort records by value.
	 * @param {string} sortdir Specify the sort direction, ascending or descending. By default, queries sort records in descending order. Set this parameter to asc to sort in ascending order.
	 ...
	 * @param {string} exclude_reference_link Set to true to hide additional information provided for reference fields, such as the URI to the reference resource.
	 * @param {string} include_scores Set to true to return indicator scores for the entire time range selected on the Analytics Hub. If a value is not specified, this parameter defaults to false and returns only the most recent score value.
	 * 
	 * To constrain the date range of the scores that are returned, combine this parameter with the from and to parameters.
	 * @param {string} from Specify the earliest date to return scores from. Only scores from this date or later are returned. The date format must match the ISO-8601 standard.
	 * 
	 * This parameter requires that include_scores is set to true.
	 * @param {string} to Specify the latest date to return scores from. Only scores from this date or earlier are returned. The date format must match the ISO-8601 standard.
	 * 
	 * This parameter requires that include_scores is set to true.
	 * @param {string} step Specify a numeric value to skip scores, based on the indicator frequency. For example, specify a value of 3 to return only scores from every third day for a daily indicator, or from every third week for a weekly indicator.
	 * @param {string} limit Specify the maximum number of scores to return.
	 * @param {string} include_available_breakdowns Set to true to return all available breakdowns for an indicator. If a value is not specified, this parameter defaults to false and returns no breakdowns.
	 * @param {string} include_available_aggregates Set to true to return all possible aggregates for an indicator, including aggregates that have already been applied. If a value is not specified, this parameter defaults to false and returns no aggregates.
	 * @param {string} include_realtime Set this parameter to true to return the realtime_enabled element which indicates if real-time scores are enabled for the indicator, and the realtime_value element which contains the real-time score value. This parameter is not supported for formula indicators.
	 * @param {string} include_target_color_scheme Set this parameter to true to return the target_color_scheme element that contains the minimum and maximum values, and the color of each section of the target color scheme for the Analytics Hub.
	 * @param {string} include_forecast_scores Set this parameter to true to return the forecast_scores element that contains an array of date-value pairs that define the forecast data for the Analytics Hub.
	 * 
	 * This paramater requires that the include_scores parameter is also set to true.
	 * @param {string} include_trendline_scores Set this parameter to true to return the trendline_scores element that contains an array of date-value pairs that define the Analytics Hub trendline.
	 * 
	 * This paramater requires that the include_scores parameter is also set to true.
	 *
	 * @returns {void} Method does not return a value
...
/** 
 * This API requires the Remote Tables plugin (com.glide.script.vtable) to be activated. For more information on Remote Tables, see [Retrieving external data using remote tables and scripts](https://docs.servicenow.com/bundle/orlando-servicenow-platform/page/administer/remote-tables/concept/remote-tables.html).  
 *   
 * Remote table rows are created using the [v_table](dev.do#!/reference/api/orlando/server/no-namespace/v_tableAPI "Scriptable object that enables you to add rows to a remote table.") API.
 * 
 */
declare class v_queryAPI {
	 ...
	 *
	 * @returns {string} Returns an [encoded query string](https://docs.servicenow.com/bundle/orlando-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html) for the given field.
	 */
	 ...
	 *
	 * For details, see [Encoded query strings](https://docs.servicenow.com/bundle/orlando-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html).
	 *
	 *
	 * @returns {string} The encoded query as a string.
	 */
	 ...
	 * See also:
	 * 	 * 
	 * 	 * *   [Encoded query strings](https://docs.servicenow.com/bundle/orlando-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html)
	 * 	 * *   [Querying tables in script](https://docs.servicenow.com/bundle/orlando-application-development/page/script/server-scripting/concept/c_UsingGlideRecordToQueryTables.html)
	 *
	 *
	 ...
	 *
	 * See also [Scoped GlideRecord - getLastErrorMessage()](dev.do#!/reference/api/orlando/server/no-namespace/c_GlideRecordScopedAPI "Scoped GlideRecord is used for database operations.").
	 *
 ...
 * This API requires the Remote Tables plugin (com.glide.script.vtable) to be activated. For more information on Remote Tables, see [Retrieving external data using remote tables and scripts](https://docs.servicenow.com/bundle/orlando-servicenow-platform/page/administer/remote-tables/concept/remote-tables.html).  
 *   
 * Use the [v_query](dev.do#!/reference/api/orlando/server/no-namespace/v_queryAPI "Scriptable object that represents a query running against a remote table.") scriptable object to query remote tables.
 * 
 */
declare class v_tableAPI {
	 ...
	 *
	 * See also [Create a script definition for a remote table](https://docs.servicenow.com/bundle/orlando-servicenow-platform/page/administer/remote-tables/task/create-remote-table-script.html).
	 *
