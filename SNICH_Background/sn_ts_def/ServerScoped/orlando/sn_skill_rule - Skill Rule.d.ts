declare namespace sn_skill_rule - Skill Rule {
	...
	class SkillDeterminationUtils {
		 ...
		 * @param {GlideRecord} gr GlideRecord of the work item on which to assign skills.
		 *
		 * @returns {void} 
		 */
		assignSkillsToWorkItem(skills: {[fieldName: string]: string}, gr: GlideRecord): void
		
		/**
		 *
		 * Gets skills for a specified work item, indicates if the skills are mandatory, and lists any skill levels.
		 *
		 * @param {GlideRecord} gr GlideRecord of a work item from any interaction or task table extension.
		 *
		 ...
		 */
		determineWorkItemSkills(gr: GlideRecord): {[fieldName: string]: string}
