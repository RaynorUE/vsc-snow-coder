declare namespace sn_notification {
	 ...
	 *   
	 * Use this API with the [Preferences](dev.do#!/reference/api/rome/server/sn_notification-namespace/PreferencesBothAPI "The Preferences API gets notification destinations for a user.") API. Before calling any methods in this class, you must call the Preferences.getDestinations() or Preferences.getDestinationsByChannel() methods to instantiate a PreferenceDestination object.  
	 *   
	 ...
	 * A notification destination is somewhere that a notification can be delivered to, such as a specific email address or phone number. This API is based on notifications from the Notification [sys_notification] table. Notifications are sent through channels such as email or Workspace. A channel can be used to send notifications to multiple types of destinations. For example, an email channel could send notifications to both personal email and work email destinations. Destination types are listed in the Notification Destination Type [sys_notif_destination_type] table.  
	 *   
	 * This API is used with the [PreferenceDestination](dev.do#!/reference/api/rome/server/ "The PreferenceDestination API updates user notification preferences.") API to update user notification preferences.  
	 *   
	 * This class uses the sn_notification namespace identifier.
	 * 
	 */
	class Preferences {
		 ...
		 *
		 *
		 * @returns {{[fieldName: string]: string}} Array of [PreferenceDestination](dev.do#!/reference/api/rome/server/ "The PreferenceDestination API updates user notification preferences.") objects. If the user doesn't have any destinations, the array is empty.
		 */
		 ...
		 *
		 * @returns {{[fieldName: string]: string}} Array of [PreferenceDestination](dev.do#!/reference/api/rome/server/ "The PreferenceDestination API updates user notification preferences.") objects. If the user doesn't have any notification destinations that use the channel or if the channel doesn't exist, the array is empty.
		 */
