declare namespace sn_agent {
	...
	/** 
	 * This API requires the Agent Client Collector Framework (sn_agent) store application and is provided within the sn_agent namespace. For more information, refer to [Agent Client Collector](https://docs.servicenow.com/bundle/rome-it-operations-management/page/product/agent-client-collector/concept/acc-landing-page.html).  
	 *   
	 * For the REST API solution, refer to [Agent Client Collector API](https://docs.servicenow.com/bundle/rome-application-development/page/integrate/inbound-rest/concept/agnt_clnt_cll-api.html).  
	 *   
	...
	class AccAgentsAPI {
		 ...
		 *
		 * Run the [submitGrabLogRequest()](dev.do#!/reference/api/rome/server/ "Requests the log of a specified agent with alive/up status.") method to get a request ID.
		 *
		 ...
		 * 		 * *   Run the [Agent Client Collector GET list](dev.do#!/reference/api/rome/server/agnt_clnt_cll-api#agnt_clnt_cll-GET-list "Gets a list of agents with related information.") REST API.
		 *
		 ...
		 *
		 * @param {string} encodedQuery Encoded query string in standard Glide format. See [Encoded query strings](https://docs.servicenow.com/bundle/rome-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html).
