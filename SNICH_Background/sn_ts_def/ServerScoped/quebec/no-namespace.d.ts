/** 
 * Use the action API to configure UI actions with which users can interact. Use these scripts in the UI Action [sys_ui_action] table. For information, see [UI actions](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/list-administration/task/t_EditingAUIAction.html).
 * 
...
declare class ActionAPIBoth {
	 ...
	 * 
	 * *   [GlideURI](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideURIScopedAPI "Enables handling the URI parameter in scoped applications.")
	 * *   [GlideURI - Global](https://developer.servicenow.com/go_to_api.do?ID=GlideURIGlobalAPI;v=quebec)
	 */
	 ...
	 * For use cases:
	 * 	 * 
	 * 	 * *   [Create a UI routing action](https://docs.servicenow.com/bundle/quebec-application-development/page/script/useful-scripts/task/t_CreateAUIRoutingAction.html)
	 * 	 * *   [Create knowledge articles from HR cases](https://docs.servicenow.com/bundle/quebec-servicenow-platform/page/product/knowledge-management/task/ui-action-hrcase-article.html)
	 * 	 * *   [Define task relationships with UI actions](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/task-table/reference/r_UIActions.html)
	 *
	 * @param {{[fieldName: string]: string}} URL URL to set as the redirect. You can provide the URL as a string or a [GlideRecord](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI "Scoped GlideRecord is used for database operations."). If you pass the URL as a GlideRecord, this value takes the focus to that record's form.
	 *
	 * @returns {void} Method does not return a value
	 */
	 ...
	 *
	 * @param {{[fieldName: string]: string}} URL URI to set as the return location after a UI action is complete. You can provide the URL as a string or a [GlideRecord](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI "Scoped GlideRecord is used for database operations.").
	 *
	 * @returns {void} Method does not return a value
	 */
 ...
 * 
 */
declare class FlowScriptAPI - Scoped {
...
declare class GlideAggregate {
	 ...
	 *
	 * @param {string} agg Type of aggregate.
	 * 
	 * Valid values:
	 * 
	 * *   AVG
	 ...
	 * *   STDDEV
	 * *   SUM
	 * @param {string} name Name of the field on which to obtain the aggregation.
	 *
	 * @returns {string} Value of the aggregation.
	 * 
 ...
 *   
 * For additional information on domain-specific application properties, see [Domain-separated application properties](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/company-and-domain-separation/concept/ds-application-properties.html).
 * 
 */
...
declare class GlideCalendarDateTime {
	...
	/**
	 *
	 * Returns the calendar date/time value in the user or system time zone based on the value specified in the Time zone field of the corresponding Business Calendar record.
	 *
	 * Note: The Start and End fields in the Business Calendar Entry [business_calendar_span] table contain an attribute time_zone_field that is set to the value in the Time zone field (calendar.time_zone).
	 * 	 * 
	 * 	 * For details on creating a business calendar and setting the time zone value, see [Create a business calendar](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/time/task/create-business-calendar.html).
	 *
	 *
	 * @returns {string} Date and time in the internal format and time zone based on the Time zone field of the corresponding Business Calendar record.
	 * 
	 * *   Floating: User time zone
	 * *   Any other value: System time zone
	 */
	 ...
	 *
	 * @param {{[fieldName: string]: string}} timeZone Time zone object
	 *
	 * @returns {void} Method does not return a value
	 */
	setTZ(timeZone: {[fieldName: string]: string}): void
...
declare class GlideDuration {
	 ...
	 * 
	 * Format: [Global date and time field format](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/time/reference/r_FormatDateAndTimeFields.html)
	 *
...
declare class GlideElementCurrency2 {
	 ...
	 *
	 * For additional information on FX Currency fields, see [Setting up and operating FX Currency fields](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/currency/concept/fx-currency.html).
	 *
	 *
	 ...
	 * 	 * 
	 * 	 * For additional information on FX Currency and the configuration table, see [Configure FX Currency global settings](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/currency/task/fx-currency-configure.html).
	 *
	 *
	 ...
	 * 	 * 
	 * 	 * For additional information on the values in the FX Currency Configuration [fx_configuration] table, see [Setting up and operating FX Currency fields](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/currency/concept/fx-currency.html).
	 *
	 *
 ...
 * ----------------
 * 
 * GlideFilter is case-sensitive by default. Use the [setCaseSensitive()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideFilterScopedAPI#SGF-setCaseSensitive_B "Enables or disables case-sensitive filter results.") method to enable or disable case sensitivity. [GlideRecord](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI "Scoped GlideRecord is used for database operations.") queries are case-insensitive.
 * 
...
declare class GlideFilter {
	...
	/**
	 *
	 * @param {string} filter Encoded query string in standard Glide format. See [Encoded query strings](https://docs.servicenow.com/bundle/quebec-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html). Results are case-sensitive, unless disabled using the [setCaseSensitive()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideFilterScopedAPI#SGF-setCaseSensitive_B "Enables or disables case-sensitive filter results.") method.
	 * 
	 ...
	 * @param {string} filter Encoded query string in standard Glide format. See [Encoded query strings](https://docs.servicenow.com/bundle/quebec-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html).
	 * 
	 * To exclude null values from GlideFilter query results, add ISNOTEMPTY to the query condition.
 ...
 *   
 * For an example on using this class, [Example: g_scratchpad](https://developer.servicenow.com/dev.do#!/guides/quebec/now-platform/tpb-guide/client_scripting_technical_best_practices%23%23example-g-scratchpad) on the Developer Site.
 * 
 */
 ...
 *           .get()
 *     
 *     See the [where()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-where_S_S "Adds a Where clause to the query that returns values based on a given condition.") method to understand how to nest a child query instead.
 *     
 * 
...
declare class GlideQuery {
	 ...
	 *
	 * After transforming the query, use the query() method in the [GlideRecord](dev.do#!/reference/api/quebec/server/c_GlideRecordAPI "The GlideRecord API is used for database operations.") or [GlideAggregate](dev.do#!/reference/api/quebec/server/c_GlideAggregateAPI "GlideAggregate enables creating database aggregation queries.") classes to query the database.
	 *
	 *
...
declare class GlideQueryCondition {
	 ...
	 *
	 * addOrCondition() works in conjunction with any of the [addQuery()](https://developer.servicenow.com/go_to_api.do?ID=r_ScopedGlideRecordAddQuery_String_String_Object&v=quebec) methods to OR the specified query parameters to the query previously constructed using addQuery().
	 * 	 * 
 ...
 * 
 * For information on a class that performs the same functions as GlideRecord and enforces ACLs, see [Using GlideRecordSecure](https://docs.servicenow.com/bundle/quebec-application-development/page/script/glide-server-apis/topic/p_GlideServerAPIs.html#concepthrq5bcgp).
 * 
 * Always test queries on a sub-production instance prior to deploying them on a production instance. An incorrectly constructed encoded query, such as including an invalid field name, produces an invalid query. When the invalid query is run, the invalid part of the query condition is dropped, and the results are based on the valid part of the query, which may return all records from the table. Using an insert(), update(), deleteRecord(), or deleteMultiple() method on bad query results can result in data loss.
 * 
 * You can set the glide.invalid_query.returns_no_rows system property to true to have queries with invalid encoded queries return no records.
 ...
 * 
 * *   [getValue()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordGetValue_String "Retrieves the string value of an underlying element in a field.")
 * *   [getDisplayValue()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordGetDisplayValue "Retrieves the display value for the current record.")
 * 
 ...
 * See also:
 * 
 * *   [GlideAggregate](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideAggregateScopedAPI "GlideAggregate enables creating database aggregation queries.")
 * *   [GlideElement](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideElementScopedAPI "The Scoped GlideElement API provides a number of convenient script methods for dealing with fields and their values. Scoped GlideElement methods are available for the fields of the current GlideRecord.")
 * *   [GlideQuery](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI "The GlideQuery API is an alternative to GlideRecord to perform CRUD operations on record data from server-side scripts.")
 * 
 */
declare class GlideRecord {
	 ...
	 * 	 * You can set the glide.invalid_query.returns_no_rows system property to true to have queries with invalid encoded queries return no records.
	 *
	 * @param {string} query An [encoded query string](https://docs.servicenow.com/bundle/quebec-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html).
	 *
	 * @returns {void} Method does not return a value
	 */
	addEncodedQuery(query: string): void
	 ...
	 * 	 * You can set the glide.invalid_query.returns_no_rows system property to true to have queries with invalid encoded queries return no records.
	 *
	 * @param {string} query An [encoded query string](https://docs.servicenow.com/bundle/quebec-platform-administration/page/use/using-lists/concept/c_EncodedQueryStrings.html) .
	 *
	 * @returns {GlideQueryCondition} The query condition added to the GlideRecord.
	 */
	 ...
	 * @param {number} lastRow Last row to include in the range. Because the index starts at 0, use the value _n_ - 1, in which _n_ equals the actual row number.
	 * @param {boolean} forceCount Optional. Not currently implemented.
	 *
	 * @returns {void} Method does not return a value
	 */
	 ...
	 * 	 * See also:
	 * 	 * 
	 * 	 * *   [GlideElement – toString()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideElementScopedAPI#r_ScopedGlideElementToString "Converts the value of a GlideRecord field to a string.")
	 * 	 * *   [getElements()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI#SGR-getElements "Returns an array of GlideElement objects. Each object describes a field in the current GlideRecord.")
	 *
	 ...
	 * 	 * 
	 * 	 * *   [GlideElement – toString()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideElementScopedAPI#r_ScopedGlideElementToString "Converts the value of a GlideRecord field to a string.")
	 * 	 * *   [getElement()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordGetElement_String "Retrieves the GlideElement object for the specified field.")
	 *
	 *
	 * @returns {{[fieldName: string]: string}} Array of [GlideElement](dev.do#!/reference/api/quebec/server/c_GlideElementAPI "The GlideElement API provides a number of convenient script methods for dealing with fields and their values. GlideElement methods are available for the fields of the current GlideRecord.") objects. Each object describes a field in the current GlideRecord.
	 */
	 ...
	 *
	 * For details, see [Encoded query strings](https://docs.servicenow.com/bundle/quebec-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html).
	 *
	 *
	 * @returns {string} 
	 */
	 ...
	 *
	 * Call this method more than once to order by multiple columns. Results are arranged in ascending order. To arrange records in descending order, see [Scoped GlideRecord - orderByDesc(String name)](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordOrderByDesc_String "Specifies a descending orderBy column.").
	 *
	 ...
	 *
	 * Call this method more than once to order by multiple columns. Results are arranged in descending order. To arrange records in ascending order, see [Scoped GlideRecord - orderBy(String name)](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI#r_ScopedGlideRecordOrderBy_String "Specifies an orderBy column.").
	 *
	 ...
	 *
	 * This method queries the GlideRecord table as well as any references of the table. For more information, see [Querying tables in script](https://docs.servicenow.com/bundle/quebec-application-development/page/script/server-scripting/concept/c_UsingGlideRecordToQueryTables.html).
	 * 	 * 
	 * 	 * Note: This method fails if there is a field in the table called "query". If that is the case, use the[_query()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI#SGR-_query_O_O "Runs a query against the table based on the filters specified by query methods such as addQuery() and addEncodedQuery(). This method is intended to be used on tables in which there is a column named "query", which might cause errors running the query() method.") method instead.
	 * 	 * 
	 * 	 * See the [GlideRecord](https://developer.servicenow.com/dev.do#!/learn/courses/quebec/app_store_learnv2_scripting_quebec_scripting_in_servicenow/app_store_learnv2_scripting_quebec_server_side_scripting/app_store_learnv2_scripting_quebec_gliderecord) article for details on building and running queries.
	 * 	 * 
	 * 	 * See also:
	 ...
	 * 	 * *   [GlideAggregate](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideAggregateScopedAPI "GlideAggregate enables creating database aggregation queries.")
	 * 	 * *   [GlideQuery](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI "The GlideQuery API is an alternative to GlideRecord to perform CRUD operations on record data from server-side scripts.")
	 * 	 * *   [GlideQueryCondition](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideQueryConditionScopedAPI "The scoped GlideQueryCondition API provides additional AND or OR conditions that can be added to the current condition, allowing you to build complex queries.")
	 *
	 ...
	 *
	 * Normally the script does a now_GR.category = value. However, if the element name is itself a variable then now_GR.setValue(elementName, value) can be used. When setting a value, ensure the data type of the field matches the data type of the value you enter.
	 * 	 * 
	 ...
	 *
	 * @param {string} name Name of the field.
	 * @param {{[fieldName: string]: string}} value The value to assign to the field.
	 *
	 * @returns {void} Method does not return a value
	 */
...
declare class GlideStringUtil {
	 ...
	 *
	 * @param {string} htmlString The string to be processed.
	 *
...
declare class GlideSysAttachment {
	 ...
	 *
	 * You can use the [GlideTextReader](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideTextReaderScopedAPI "Provides the ability to read single lines from an input stream. Because an input stream is used, it is not subject to the 5MB attachment size limit.") API to read the content stream.
	 *
...
declare class GlideSystem {
	 ...
	 *
	 * The passed in event schedule information is stored in the Events [sysevent] table. For additional information on events, see [Events](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/platform-events/concept/events.html).
	 *
	 ...
	 *
	 * This method has been deprecated. Instead, use the [getTimeZoneName()](https://developer.servicenow.com/go_to_api.do?ID=r_ScopedGlideSessionGetTimeZoneName&v=quebec) method in the GlideSession API.
	 *
	 *
	 ...
	 *
	 * See [GlideUser](dev.do#!/reference/api/quebec/server/c_GlideUserAPI "The GlideUser API provides access to information about the current user and current user roles. Using the GlideUser API avoids the need to use the slower GlideRecord queries to get user information.") for a list of available methods.
	 *
	 *
	...
	/**
	 *
	 * Provides a safe way to call from the sandbox, allowing only the inclusion of trusted scripts.
	 *
	 * @param {string} name The name of the script to include.
	 *
...
declare class GlideTime {
	 ...
	 *
	 * For information on the time formats that you can pass into this method, see [Global date and time field format](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/time/reference/r_FormatDateAndTimeFields.html). You can pass these values in any order and add separators such as a dash, space, backslash, or colon. In addition, you can also pass the character "a" to return am/pm.
	 *
 ...
 * 
 * *   [Navigate by URL](https://docs.servicenow.com/bundle/quebec-platform-user-interface/page/use/navigation/task/navigate-using-url.html)
 * *   [Navigation stack](https://docs.servicenow.com/bundle/quebec-platform-user-interface/page/administer/navigation-and-ui/concept/c_NavigationStack.html)
 * 
 *   
 *   
 * See also: [GlideURI -Global](https://developer.servicenow.com/go_to_api.do?ID=GlideURIGlobalAPI;v=quebec)
 * 
 */
 ...
 *   
 * 
 * *   Return an Optional object from these methods in the GlideQuery class. For more information, see [GlideQuery](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI "The GlideQuery API is an alternative to GlideRecord to perform CRUD operations on record data from server-side scripts.").
 *     *   [getBy()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-getBy_O_O "Returns an Optional object containing a single record based on a set of name-value pairs to query by. Assumes the '=' operator for each name-value pair.")
 *     *   [get()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-get_S_O "Returns a single record from the query.")
 *     *   [insert()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-insert_O_O "Inserts a record and returns an Optional object containing the record.")
 *     *   [insertOrUpdate()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-insertOrUpdate_O_O "Updates an existing record, or inserts a new record if one does not already exist.")
 *     *   [update()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-update_O_O "Updates an existing record that matches the defined conditions.")
 *     *   [selectOne()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-selectOne_S "Returns the result of the query as an Optional object containing specified fields.")
 *     *   [avg()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-avg_S "Returns the aggregate average of a given numeric field.")
 *     *   [max()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-max_S "Returns the aggregate maximum of a given field.")
 *     *   [min()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-min_S "Returns the aggregate minimum of a given field.")
 *     *   [sum()](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI#GQ-sum_S "Returns the aggregate sum of a given numeric field.")
 * *   Return an Optional object from the [find()](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI#Stream-find_F "Returns the first record or item in the Stream object that matches the predicate function. If no predicate function is provided, then the method returns the first record or item in the Stream.") method in the Stream class. For more information on Stream, see the [Stream](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI "The Stream API interacts with a stream of items such as records. For example, you can use the forEach() method to update the state of each record in a stream returned by the GlideQuery API.") API.
 * *   Use the [lazy()](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI#Optional-lazy_F "Returns a new Optional object. Instead of containing the record, the object contains a function to get the record that is only called if and when requested in the code.") method to generate the value of the Optional if ever needed.
 * 
 *   
 *   
 ...
 *   
 * 
 * *   [lazy()](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI#Optional-lazy_F "Returns a new Optional object. Instead of containing the record, the object contains a function to get the record that is only called if and when requested in the code.")
 * *   [of()](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI#Optional-of_A "Wraps a given value in an Optional object. For example, you can wrap the result of a GlideRecord query in an Optional object to use the associated methods.")
 * *   [empty()](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI#Optional-empty_S "Returns an empty Optional object. Use this method in an Else clause to handle a query that might not return a result.")
 * 
 *   
 *   
 * You can use these static methods with any API that returns a single value such as [GlideRecord](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI "Scoped GlideRecord is used for database operations.").  
 *   
 ...
 * --------------
 * 
 * This API can work with the [GlideQuery](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI "The GlideQuery API is an alternative to GlideRecord to perform CRUD operations on record data from server-side scripts.") and [Stream](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI "The Stream API interacts with a stream of items such as records. For example, you can use the forEach() method to update the state of each record in a stream returned by the GlideQuery API.") APIs in a builder pattern where the method calls chain together, each method building on the returned result of the previous method. Use methods to define the attributes of the query. The methods do not execute until you call a terminal method, a method that returns a query result, allowing you to define the requirements of the query before executing it.
 * 
 * If the query returns a single record, the system wraps the result in an Optional object. If the query returns a stream of records, the system wraps the result in a Stream object. These objects let you manage the result using a set of methods in each API.
 * 
 * For example, here's a script that performs a query on the Task table and groups the records by priority and returns groups with total reassignments greater than four.
 ...
 * 
 * *   [get()](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI#Optional-get "Returns the record inside the Optional object, or returns an error if the query does not return a record.")
 * *   [orElse()](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI#Optional-orElse "Adds a default value within the Optional object if the query does not return any results.")
 * *   [ifPresent()](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI#Optional-ifPresent_F "Applies a function to the record within an Optional object. If the Optional object does not contain a record, the function does not execute.")
 * *   [isEmpty()](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI#Optional-isEmpty "Returns true if the Optional object is empty.")
 * *   [isPresent()](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI#Optional-isPresent "Checks whether an Optional object contains a value.")
 * 
 */
...
/** 
 * This API requires the Performance Analytics (com.snc.pa.analytics_center) application. For more information, see [Performance Analytics](https://docs.servicenow.com/bundle/quebec-now-intelligence/page/use/performance-analytics/reference/r_PALandingPage.html).
 * 
 */
declare class PAScorecard {
	 ...
	 * 
	 * For information about obtaining the sys_id values of records, see [Unique record identifier (sys_id)](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/table-administration/concept/c_UniqueRecordIdentifier.html).
	 * 
 ...
 *   
 * For more information, see [Data Classification](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/security/concept/data-classification.html).
 * 
 */
 ...
 * *   Return a Stream object from the GlideQuery.select() method. For more information, see [GlideQuery](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI "The GlideQuery API is an alternative to GlideRecord to perform CRUD operations on record data from server-side scripts.").
 * 
 *   
 *   
 * This method is static and does not require an instance of the class: [fromArray()](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI#Stream-fromArray_O "Returns a Stream object that contains the values from the provided array.").  
 *   
 ...
 * --------------
 * 
 * This API can work with the [GlideQuery](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI "The GlideQuery API is an alternative to GlideRecord to perform CRUD operations on record data from server-side scripts.") and [Optional](dev.do#!/reference/api/quebec/server/no-namespace/OptionalGlobalAPI "The Optional API interacts with a single record returned by the GlideQuery, Stream, or GlideRecord APIs, even when it does not exist. Write scripts that are less likely to result in an error by handling null or undefined query results.") APIs in a builder pattern where the method calls chain together, each method building on the returned result of the previous method. Use methods to define the attributes of the query. The methods do not execute until you call a terminal method, a method that returns a query result, allowing you to define the requirements of the query before executing it.
 * 
 * If the query returns a single record, the system wraps the result in an Optional object. If the query returns a stream of records, the system wraps the result in a Stream object. These objects let you manage the result using a set of methods in each API.
 * 
 * For example, here's a script that performs a query on the Task table and groups the records by priority and returns groups with total reassignments greater than four.
 ...
 * 
 * *   [every()](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI#Stream-every_F "Applies a predicate function to every item in the Stream object. If the predicate returns true for every item in the stream, the method returns true. If the predicate returns false for any item in the stream, the method returns false.")
 * *   [find()](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI#Stream-find_F "Returns the first record or item in the Stream object that matches the predicate function. If no predicate function is provided, then the method returns the first record or item in the Stream.")
 * *   [forEach()](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI#Stream-forEach_F "Applies the specified function to each record or item in the stream.")
 * *   [reduce()](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI#Stream-reduce_F_A "Executes a reducer function on each item in the stream, resulting in single output value.")
 * *   [some()](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI#Stream-some_F "Applies a predicate function, a function that takes a single value and returns true or false, to each item in the stream. If the predicate returns true for any item in the stream, the method returns true.")
 * *   [toArray()](dev.do#!/reference/api/quebec/server/no-namespace/StreamGlobalAPI#Stream-toArray_n "Returns an array containing the given number of items from the stream.")
 * 
 */
declare class Stream {
	 ...
	 *
	 * For better performance, use the where(), whereNotNull(), and whereNull() methods in the GlideQuery class instead of this method where possible. See [GlideQuery](dev.do#!/reference/api/quebec/server/no-namespace/GlideQueryAPI "The GlideQuery API is an alternative to GlideRecord to perform CRUD operations on record data from server-side scripts.").
	 *
	 * @param {{[fieldName: string]: string}} predicate Predicate function to apply to every record or item inside the Stream object. The function must take each item in the stream as input and return a boolean.
	 *
	 * @returns {{[fieldName: string]: string}} Object used to interact with a stream of items such as records.
...
/** 
 * This API requires the Remote Tables plugin (com.glide.script.vtable) to be activated. [Retrieving external data using remote tables and scripts](https://docs.servicenow.com/bundle/quebec-servicenow-platform/page/administer/remote-tables/concept/remote-tables.html).  
 *   
 * Remote table rows are created using the [v_table](dev.do#!/reference/api/quebec/server/no-namespace/v_tableAPI "Scriptable object that enables you to add rows to a remote table.") API.
 * 
 */
declare class v_queryAPI {
	 ...
	 *
	 * @returns {string} Returns an [encoded query string](https://docs.servicenow.com/bundle/quebec-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html) for the given field.
	 */
	 ...
	 * See also:
	 * 	 * 
	 * 	 * *   [Encoded query strings](https://docs.servicenow.com/bundle/quebec-platform-user-interface/page/use/using-lists/concept/c_EncodedQueryStrings.html)
	 * 	 * *   [Querying tables in script](https://docs.servicenow.com/bundle/quebec-application-development/page/script/server-scripting/concept/c_UsingGlideRecordToQueryTables.html)
	 *
	 *
	 ...
	 *
	 * See also [Scoped GlideRecord - getLastErrorMessage()](dev.do#!/reference/api/quebec/server/no-namespace/c_GlideRecordScopedAPI "Scoped GlideRecord is used for database operations.").
	 *
 ...
 * This API requires the Remote Tables plugin (com.glide.script.vtable) to be activated. [Retrieving external data using remote tables and scripts](https://docs.servicenow.com/bundle/quebec-servicenow-platform/page/administer/remote-tables/concept/remote-tables.html).  
 *   
 * Use the [v_query](dev.do#!/reference/api/quebec/server/no-namespace/v_queryAPI "Scriptable object that represents a query running against a remote table.") scriptable object to query remote tables.
 * 
 */
declare class v_tableAPI {
	 ...
	 *
	 * See also [Create a script definition for a remote table](https://docs.servicenow.com/bundle/quebec-servicenow-platform/page/administer/remote-tables/task/create-remote-table-script.html).
	 *
