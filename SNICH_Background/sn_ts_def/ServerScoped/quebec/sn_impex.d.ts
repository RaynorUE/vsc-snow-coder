declare namespace sn_impex {
	...
	class GlideExcelParser {
		 ...
		 *
		 *
		 * @returns {{[fieldName: string]: string}} Array of strings as worksheet names ordered as positioned from left to right in the workbook. Results include any worksheets marked as hidden.
		 */
		getSheetNames(): {[fieldName: string]: string}
		 ...
		 *
		 * @param {{[fieldName: string]: string}} inputStream Excel document provided as an input stream.
		 * 
		 * Note: Do not set this value if using the [setSource()](dev.do#!/reference/api/quebec/server/sn_impex-namespace/GlideExcelParserScopedAPI#GEPS-setSource_O "Defines an input source for parsing multiple times or parsing each worksheet in an Excel file.") method to parse the same source multiple times.
		 *
		 * @returns {boolean} Flag that indicates if the parse was successful.
		 * 
		 * Valid values:
		 * 
		 ...
		 */
		parse(inputStream: {[fieldName: string]: string}): boolean
		 ...
		 * @returns {void} 
		 */
		setSource(inputStream: {[fieldName: string]: string}): void
