declare namespace sn_cmdb {
	...
	class IdentificationEngine {
		...
		/**
		 *
		 * Insert or update configuration items and non-Configuration Management Database (CMDB) configuration items (classes not extending from cmdb_ci) in the CMDB based on identification and reconciliation rules. Use this API instead of updating the CMDB directly.
		 *
		 ...
		 * }
		 * @param {string} input.items.settings.updateWithoutDowngrade Flag that indicates whether update and downgrade are both permitted for this item.
		 * 
		 * Valid values:
		 * 
		 ...
		 * 
		 * Default: false
		 * @param {string} input.items.settings.updateWithoutUpgrade Flag that indicates whether update and upgrade are both permitted for this item.
		 * 
		 * Valid values:
		 * 
		 ...
		 * 
		 * Default: false
		 * @param {string} input.items.settings.updateWithoutSwitch Flag that indicates whether the item can be updated and the class switched.
		 * 
		 * Valid values:
		 * 
		 ...
		 * 
		 * A node in this array can use either of two formats.
		 * 
		 * *   The node can define a relationship between two top-level items (only) using parent and child name-value pairs, with values representing item indexes from the payload items array.
		 * *   The node can define a relationship between any two items, including top-level, related, or lookup items, using parent_id and child_id key/value pairs, with values representing internal_id values defined for those items.
		 * 
		 * "relations": [
		 *   {
		 *     "child": Number,
		 ...
		 *   }
		 * ]
		 * @param {number} input.relations.child Integer index of the CI object in the items array that represents the child in the relationship.
		 * @param {string} input.relations.child_id The internal_id of the child item in the relation.
		 * @param {number} input.relations.parent Integer index of the parent item in the items array.
		 * @param {string} input.relations.parent_id The internal_id of the parent item in the relation (item.related or item.lookup item).
		 * @param {string} input.relations.type The type of relationship that exists between the parent and child items. This must be a name field value from the CI Relationship Type [cmdb_rel_type] table.
		 ...
		 *
		 * @returns {any} The sys_id of the dependent relationship CI.
		 * 
		 * Data type: String
		 */
		 ...
		 * 
		 * Default: false
		 * @param {boolean} options.partial_commits Flag that indicates whether partial commit support is enabled. For additional information on partial commits, see [Enhanced IRE features](https://docs.servicenow.com/bundle/paris-servicenow-platform/page/product/configuration-management/concept/ire.html).
		 * 
		 * Valid values:
		 * 
		 ...
		 * *   errors - a list of errors in the format of (error, message string)
		 ...
		 * 		 * 
		 * 		 * This method is similar to the [IdentificationEngine - identifyCI(String jsonString)](dev.do#!/reference/api/paris/server/sn_cmdb-namespace/IdentificationEngineScopedAPI#IESS-identifyCI_S "Determines the operation (insert/update) to perform with the specified payload without committing the operation in the database.") method, however it also supports the following functionality:
		 * 		 * 
		 * 		 * *   Partial payloads
