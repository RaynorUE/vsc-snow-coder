declare namespace sn_cti_core {
	...
	class CTIOperationRequest {
		 ...
		 * 		 * 
		 * 		 * The [CTIOperationRequest - getInteractionRecord()](https://developer.servicenow.com/go_to_api.do?ID=CTIOpReq-getInteractionRecord&v=paris) method uses this sys_id to obtain the correct interaction record for the contact session. Interaction records are located in the Interaction [interaction] table. For additional information on interaction records, see
		 *
	...
	class CTIOperationResponse {
		 ...
		 *
		 * Using the get/set parameter methods enables the passing of virtually any string or number value between an operation handler and a message transformer. The parameter object can contain zero or more key/value pairs of data that directly correlate to the current operation response. The operation handler determines what key/value pairs are required as it produces these values. Use the [CTIOperationResponse - setParameter(String key, Object value)](https://developer.servicenow.com/go_to_api.do?ID=CTIOpResp-setParameter_S_O&v=paris) method to set parameters on the current CTIOperationResponse object.
		 *
		 * @param {string} key The name of the key value to return.
		 *
