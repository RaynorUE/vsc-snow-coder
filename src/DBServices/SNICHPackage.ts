...
export class SNICHPackageDB {
    ...
    constructor(logger: SNICHLogger) {
        ...
        if (!DBfilePath) {
            throw new Error('Unable to get packages! Somehow this got called without valid workspace!');
        }
    ...
    getDBFilePath(): vscode.Uri | undefined {
        ...
        if (wsRootUri) {
            dbPath = vscode.Uri.joinPath(wsRootUri, '.snich', 'db', 'packages.db');
        }
